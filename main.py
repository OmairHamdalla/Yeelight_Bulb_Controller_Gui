...
class App(ctk.CTk):
    ...
    def __init__(self):
        ...
        # for color_name, (r, g, b) in self.colors:
        #     button = ctk.CTkButton(self.colors_frame, text=color_name, command=lambda r=r, g=g, b=b: self.set_color(r, g, b))
        #     button.pack(side="left", padx=5, pady=5, expand=True, fill="x")

        num_colors_per_row = 3
        num_rows = (len(self.colors) + num_colors_per_row - 1) // num_colors_per_row  # Calculate number of rows needed
    ...
    def load_config(self):
        ...
        if os.path.exists(self.CONFIG_FILE):
            with open(self.CONFIG_FILE, 'r') as f:
                config = json.load(f)
                self.saved_ip = config.get("Light 1")
    ...
    def check_bulb(self):
        ...
        try:
            self.bulb = Bulb(ip)
            self.bulb.get_properties()  # This will raise an exception if the bulb is not reachable
